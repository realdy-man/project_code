...
# === 加载用户词典 ===
# 使用相对路径，确保本地和容器内一致
USER_DICT_PATH = os.path.join(SERVICE_D_ROOT, "model_training", "dict", "user_dict.txt")
# 由于我们没有复制dict目录，所以使用内置词典
print("[OK] 使用jieba内置词典")

# === 加载停用词表 ===
def load_stopwords():
    # 使用相对路径，确保本地和容器内一致
    candidate_paths = [
        os.path.join(SERVICE_D_ROOT, "model_training", "dict", "hit_stopwords.txt"),
        os.path.join(SERVICE_D_ROOT, "model_training", "hit_stopwords.txt")
    ]
